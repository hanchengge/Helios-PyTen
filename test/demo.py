...
[T1, rX1] = cp_als(X1, r, Omega1, maxiter=10000, printitn=100)
...
rX5 = halrtc(X1, Omega1, max_iter=100, alpha=alpha, printitn=1)
self1 = TNCP(X1, Omega1, rank=r)
self1.run()
...
from pyten.method import onlineCP, OLSGD, MAST
from pyten.tools import create  # Import the problem creation function
from pyten.tools import tenerror
import numpy as np
...
# Multi-Aspect Streaming Tensor Completion (MAST)
...
problem = 'basic'  # Define Problem As Basic Tensor Completion Problem
siz = [50, 50, 40]  # Size of the Created Synthetic Tensor
...
[X1, Omega1, sol1] = create(problem, siz, r, miss, tp)
# Other Tensor Decomposition methods: DEDICOM, PARAFAC2
from pyten.method import *
# DEDICOM
from pyten.method import dedicom  # Import
self = dedicom.DEDICOM(X1, r, printitn=1)
self.run()

...
from PyTen.method import CMTF,AirCP
import pandas as pd
import numpy as np
...
def Helios2(FileName=None,FunctionName=None,AuxMode=None,AuxFile=None,Recover=None,Omega=None,R=2,tol=1e-8,maxiter=100,init='random',printitn=0):
    ...
    if  FunctionName=='1' or FunctionName=='AirCP':
        ...
        if AuxMode==None:
            AuxMode = raw_input("Please input all the modes that have Auxiliary Similarity Matrix (separate with space. Input 'None' if no auxiliary info.)\n")
            if AuxMode!='None':
                for i in range((len(AuxMode)+1)/2):
                    Mode=int(AuxMode[i*2])
                    FileName2 = raw_input("Please input the FileName of the Auxiliary Similarity Matrix of Mode "+str(Mode)+" :\n")
                    if FileName2!='None':
        ...
        Y=None
        if AuxMode==None:
            AuxMode = raw_input("Please input all the modes that have Coupled Matrix (separate with space. Input 'None' if no coupled matrices. Allow Multiple Coupled Matrices for One Mode)\n");
            if AuxMode!='None':
                for i in range((len(AuxMode)+1)/2):
                    Mode=int(AuxMode[i*2])
                    FileName2 = raw_input("Please input the FileName of the Coupled Matrix of Mode "+str(Mode)+" (Input 'None' if no auxiliary info):\n");
                    print("\n")
                    if FileName2!='None':
                        ...
                        Mat_siz = Mat_siz+1
                        X2=sparse.coo_matrix((Mat_vals,(Mat_subs[:,0],Mat_subs[:,1])),shape=(Mat_siz[0],Mat_siz[1]))
                        if CM==None:
                            CM=Mode
                            Y=X2.toarray()
                        else:
    ...
    if type(FunctionName)==str:
        if  FunctionName=='1' or FunctionName=='AirCP':
            ...
            self=AirCP.AirCP(X,Omega1,R,tol,maxiter,simMats=simMats);
            self.run();
            Final=self.U;
            Rec=self.X;
            full = self.II.copy()
            ...
            [Final,Rec,V]=CMTF.CMTF(X,Y,CM,R,Omega,tol,maxiter,init,printitn);
            full=Final.totensor();
